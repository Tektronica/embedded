...
// drives the actual row/column GPIO pins.
//
// Standalone counterpart to arduino-nano/toy-microwave's identical KeyMatrix.h -- that project
// consumes this exact technique for one specific application; this project is where the
// technique itself lives and gets exercised on its own, the same relationship pwm/stepper have
// to the projects that actually use PWM/stepper control for something.
namespace keymatrix {
...
// arduino-nano/led-dimmer-ws2812's Button class (N consistent samples before committing a state
// change), generalized from a boolean to a 16-way raw key index. Feed it the raw key index found
// by scanning all rows/columns once (or NO_KEY if none were active); returns the key character on
// a fresh, debounced press, or '\0' if nothing new settled this scan.
class Scanner {
 public:
