...
- **ClockSet** — entering a new time-of-day, HH:MM, same calculator-style digit entry as a countdown, typed in 24-hour form (e.g. `1930` for 7:30 PM). While typing, the live preview clamps hours to 23 and minutes to 59; `#` only confirms and returns to Idle if the entered hour:minute is actually a valid 24-hour time (e.g. `2400` is refused, not silently clamped to 23:00) — otherwise it's a no-op and entry continues. `*` cancels, discarding the entry and leaving the clock unchanged.
