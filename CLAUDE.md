# fw-nano-ledStripDimmer — Project Context
...
- Repo = `[codingType]-[targetPlatform]-[projectType]` → `fw` · `nano` · `ledStripDimmer`.
  Hyphen-delimited slots; camelCase within a slot.
- A different board is a *separate* repo (`fw-esp32-ledStripDimmer`).
...
- Use the **`/commit`** command (`.claude/commands/commit.md`).
- **Local commits only**, never push. **No `Co-Authored-By`** / AI attribution. Personal repo —
  no company copyright headers on source.
- Pre-commit gate: `pio run` (0 errors/warnings) + `pio test -e native`.
